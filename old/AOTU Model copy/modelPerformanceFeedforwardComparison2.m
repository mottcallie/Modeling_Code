...
function modelPerformanceFeedforwardComparison2(predicted_RF)
%% Initialize
...
startPos = 0;
simDuration = 30;
scaleFactors = [0, 1, 5, 10];  % Scaling factors for the feedforward input (0 = no feedforward, 5 = max feedforward)
...
% Save the plot
saveas(gcf, fullfile(folder.final, 'Feedforward_Model_SettingsOL.png'));
set(gcf,'renderer','Painters')
saveas(gcf, fullfile(folder.vectors, 'Feedforward_Model_SettingsOL.svg'));
...
for kIdx = 1:nK
    ...
    for scaleIdx = 1:nScales
        ...
        [timebase, visobj_history, input_history, rotvel_history] = ...
            aotu_steering_model_FF_OL(thisTuning, noiseLevel, startPos, 'inhibitory', simDuration, thisK, alpha, scaleFactor, runSettings);
        ...
        if any(kIdx == selected_k_indices)
            ...
            visobj_plot = remove_large_jumps(rotvel_history(1, :), 180);
...
saveas(gcf, fullfile(folder.final, 'Feedforward_Scaling_ExampleRunsOL.png'));
set(gcf,'renderer','Painters')
saveas(gcf, fullfile(folder.vectors, 'Feedforward_Scaling_ExampleRunsOL.svg'));
...
generateSummaryPlots2(kValues, metrics_prob, metrics_var, metrics_ISE, metrics_IAE, ...
    obj_binned_avgs, rotvel_binned_avgs, evt, obj_bins, rotvel_bins, posBins, nK, comparisonLabel, folder, "FeedforwardOL");
