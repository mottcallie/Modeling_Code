% aotu_steering_model_FF
%
% Simulates the steering behavior of a fly using a feedforward model for the AOTU019 neuron. 
% The model incorporates visual object position, rotational velocity, and inputs from various neurons, 
% and generates corresponding output based on feedforward control scaled by a given factor.
%
% INPUTS:
...
%   runSettings      - Struct containing additional simulation parameters.
%
% OUTPUTS:
...
%   rotvel_history   - History of rotational velocity of the fly (in degrees per second) for each simulation run.
%
% DESCRIPTION:
% This function models the behavior of a Drosophila fly during steering in response to a visual object. 
% The visual object is presented at different azimuthal angles, and the fly's rotational velocity is 
% computed based on inputs from visual tuning curves of different neurons (AOTU019, AOTU025) and summed 
% neuron activity (noiSum). Feedforward control is incorporated based on the rotational velocity of the fly 
% and scaled by the input `scaleFactor`. When `scaleFactor` is 0, there is no feedforward effect.
%
% The model incorporates delays for each neuron type (AOTU019, AOTU025, DNa02), applies visual tuning 
% curves to compute inputs to the DNa02 neurons, and simulates the corresponding motor output, which drives 
% the fly's rotational velocity. Noise is added to the signal to simulate real-world variability in the 
% fly's behavior, and the rotational velocity influences the fly's perceived object position.
% 
% The function outputs the timebase, visual object position history, inputs to the DNa02 neurons, and 
% rotational velocity history for further analysis or plotting.
%
% Example usage:
%   [timebase, visobj_history, input_history, rotvel_history] = aotu_steering_model_FF(noiTuning, 0.05, 0, 'inhibitory', 10, 0.5, 1.2, runSettings);
